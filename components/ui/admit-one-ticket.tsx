// @ts-nocheck
// components/ui/admit-one-ticket.tsx — WebGL shader ticket with dithering,
// tilt + glare, perforation notch. Premium interactive ticket visual.
  ...
  setUniformValues = (updatedUniforms) => {
    ...
    Object.entries(updatedUniforms).forEach(([key, value]) => {
      ...
      if (value instanceof HTMLImageElement) {
        ...
        switch (valueLength) {
          case 2: this.gl.uniform2fv(location, flatArray); break;
          case 3: this.gl.uniform3fv(location, flatArray); break;
          case 4: this.gl.uniform4fv(location, flatArray); break;
          case 9: this.gl.uniformMatrix3fv(location, false, flatArray); break;
          case 16: this.gl.uniformMatrix4fv(location, false, flatArray); break;
        }
...
var defaultObjectSizing = { fit: "contain", scale: 1, rotation: 0, offsetX: 0, offsetY: 0, originX: 0.5, originY: 0.5, worldWidth: 0, worldHeight: 0 };
var defaultPatternSizing = { fit: "none", scale: 1, rotation: 0, offsetX: 0, offsetY: 0, originX: 0.5, originY: 0.5, worldWidth: 0, worldHeight: 0 };
var ShaderFitOptions = { none: 0, contain: 1, cover: 2 };

var declarePI = `#define TWO_PI 6.28318530718\n#define PI 3.14159265358979323846\n`;
var proceduralHash11 = `float hash11(float p) { p = fract(p * 0.3183099) + 0.1; p *= p + 19.19; return fract(p * p); }`;
var proceduralHash21 = `float hash21(vec2 p) { p = fract(p * vec2(0.3183099, 0.3678794)) + 0.1; p += dot(p, p + 19.19); return fract(p.x * p.y); }`;
var simplexNoise = `vec3 permute(vec3 x) { return mod(((x * 34.0) + 1.0) * x, 289.0); }
float snoise(vec2 v) {
...
${proceduralHash11}
${proceduralHash21}
float getSimplexNoise(vec2 uv, float t) { float noise = .5 * snoise(uv - vec2(0., .3 * t)); noise += .5 * snoise(2. * uv + vec2(0., .32 * t)); return noise; }
...
float getBayerValue(vec2 uv, int size) { ivec2 pos = ivec2(fract(uv / float(size)) * float(size)); int index = pos.y * size + pos.x; if (size == 2) return float(bayer2x2[index]) / 4.0; else if (size == 4) return float(bayer4x4[index]) / 16.0; else if (size == 8) return float(bayer8x8[index]) / 64.0; return 0.0; }
void main() {
  float t = .5 * u_time;
  ...
  if (u_shape > 3.5) {
    ...
    if (u_fit == 1.) objectBoxSize.x = min(u_resolution.x, u_resolution.y);
    else if (u_fit == 2.) objectBoxSize.x = max(u_resolution.x, u_resolution.y);
    ...
    if (u_fit == 1.) patternBoxSize.x = patternBoxRatio * min(u_resolution.x / patternBoxRatio, u_resolution.y);
    else if (u_fit == 2.) patternBoxSize.x = patternBoxRatio * max(u_resolution.x / patternBoxRatio, u_resolution.y);
    ...
    if (u_fit > 0.) shapeUV *= (patternWorldNoFitBoxWidth / patternBoxSize.x);
    shapeUV /= u_scale;
    shapeUV = graphicRotation * shapeUV;
    shapeUV += boxOrigin / patternWorldScale;
    shapeUV -= boxOrigin;
  ...
  } else if (u_shape < 2.5) { shapeUV *= .003; for (float i = 1.0; i < 6.0; i++) { shapeUV.x += 0.6 / i * cos(i * 2.5 * shapeUV.y + t); shapeUV.y += 0.6 / i * cos(i * 1.5 * shapeUV.x + t); } shape = .15 / max(0.001, abs(sin(t - shapeUV.y - shapeUV.x))); shape = smoothstep(0.02, 1., shape);
  } else if (u_shape < 3.5) { shapeUV *= .05; float stripeIdx = floor(2. * shapeUV.x / TWO_PI); float rand = hash11(stripeIdx * 10.); rand = sign(rand - .5) * pow(.1 + abs(rand), .4); shape = sin(shapeUV.x) * cos(shapeUV.y - 5. * rand * t); shape = pow(abs(shape), 6.);
  ...
  } else if (u_shape < 5.5) { float dist = length(shapeUV); float waves = sin(pow(dist, 1.7) * 7. - 3. * t) * .5 + .5; shape = waves;
  ...
  } else { shapeUV *= 2.; float d = 1. - pow(length(shapeUV), 2.); vec3 pos = vec3(shapeUV, sqrt(max(0., d))); vec3 lightPos = normalize(vec3(cos(1.5 * t), .8, sin(1.25 * t))); shape = .5 + .5 * dot(lightPos, pos); shape *= step(0., d); }
  int type = int(floor(u_type));
  float dithering = 0.0;
  switch (type) {
    case 1: dithering = step(hash21(ditheringNoiseUV), shape); break;
...
${proceduralHash21}
${declarePI}
float getUvFrame(vec2 uv, vec2 pad) { float aa = 0.0001; float left = smoothstep(-pad.x, -pad.x + aa, uv.x); float right = smoothstep(1.0 + pad.x, 1.0 + pad.x - aa, uv.x); float bottom = smoothstep(-pad.y, -pad.y + aa, uv.y); float top = smoothstep(1.0 + pad.y, 1.0 + pad.y - aa, uv.y); return left * right * bottom * top; }
...
void main() {
  ...
  switch (type) { case 1: dithering = step(hash21(ditheringNoiseUV), lum); break; case 2: dithering = getBayerValue(pxSizeUV, 2); break; case 3: dithering = getBayerValue(pxSizeUV, 4); break; default: dithering = getBayerValue(pxSizeUV, 8); break; }
...
var DitheringShapes = { simplex: 1, warp: 2, dots: 3, wave: 4, ripple: 5, swirl: 6, sphere: 7 };
var DitheringTypes = { "random": 1, "2x2": 2, "4x4": 3, "8x8": 4 };
...
function useMergeRefs(refs) {
  ...
  const refEffect = React.useCallback((instance) => {
    const cleanups = refs.map((ref) => {
      if (ref == null) return;
      if (typeof ref === "function") { const refCallback = ref; const refCleanup = refCallback(instance); return typeof refCleanup === "function" ? refCleanup : () => { refCallback(null); }; }
      ref.current = instance;
      return () => { ref.current = null; };
    });
    return () => { cleanups.forEach((refCleanup) => refCleanup?.()); };
  }, refs);
  return React.useMemo(() => {
    if (refs.every((ref) => ref == null)) return null;
    return (value) => { if (cleanupRef.current) { cleanupRef.current(); cleanupRef.current = void 0; } if (value != null) cleanupRef.current = refEffect(value); };
  }, refs);
}
...
async function processUniforms(uniformsProp) {
  ...
  const isValidUrl = (url) => { try { if (url.startsWith("/")) return true; new URL(url); return true; } catch { return false; } };
  const isExternalUrl = (url) => { try { if (url.startsWith("/")) return false; const urlObject = new URL(url, window.location.origin); return urlObject.origin !== window.location.origin; } catch { return false; } };
  Object.entries(uniformsProp).forEach(([key, value]) => {
    if (typeof value === "string") {
      if (!value) { processedUniforms[key] = getEmptyPixel(); return; }
      if (!isValidUrl(value)) return;
      const imagePromise = new Promise((resolve, reject) => {
        const img = new Image();
        if (isExternalUrl(value)) img.crossOrigin = "anonymous";
        img.onload = () => { setMinImageSize(img); processedUniforms[key] = img; resolve(); };
        img.onerror = () => reject();
        img.src = value;
      });
      imageLoadPromises.push(imagePromise);
    } else if (value instanceof HTMLImageElement) { setMinImageSize(value); processedUniforms[key] = value; }
...
var ShaderMount2 = forwardRef(
  function ShaderMountImpl({ fragmentShader, uniforms: uniformsProp, webGlContextAttributes, speed = 0, frame = 0, width, height, minPixelRatio, maxPixelCount, mipmaps, style, ...divProps }, forwardedRef) {
    ...
    useEffect(() => {
      ...
      initShader();
      return () => { shaderMountRef.current?.dispose(); shaderMountRef.current = null; };
    }, [fragmentShader]);
    useEffect(() => {
      let isStale = false;
      const updateUniforms = async () => { const uniforms = await processUniforms(uniformsProp); if (!isStale) shaderMountRef.current?.setUniforms(uniforms); };
      updateUniforms();
      return () => { isStale = true; };
    ...
    return jsx("div", { ref: mergedRef, style: width !== void 0 || height !== void 0 ? { width: typeof width === "string" && isNaN(+width) === false ? +width : width, height: typeof height === "string" && isNaN(+height) === false ? +height : height, ...style } : style, ...divProps });
  }
);
...
function colorPropsAreEqual(prevProps, nextProps) {
  for (const key in prevProps) {
    if (key === "colors") {
      ...
      if (!prevIsArray || !nextIsArray) { if (Object.is(prevProps.colors, nextProps.colors) === false) return false; continue; }
...
var defaultPreset = { name: "Default", params: { ...defaultPatternSizing, speed: 1, frame: 0, scale: 0.6, colorBack: "#000000", colorFront: "#00b2ff", shape: "sphere", type: "4x4", size: 2 } };

var Dithering = memo(function DitheringImpl({ speed = defaultPreset.params.speed, frame = defaultPreset.params.frame, colorBack = defaultPreset.params.colorBack, colorFront = defaultPreset.params.colorFront, shape = defaultPreset.params.shape, type = defaultPreset.params.type, pxSize, size = pxSize === void 0 ? defaultPreset.params.size : pxSize, fit = defaultPreset.params.fit, scale = defaultPreset.params.scale, rotation = defaultPreset.params.rotation, originX = defaultPreset.params.originX, originY = defaultPreset.params.originY, offsetX = defaultPreset.params.offsetX, offsetY = defaultPreset.params.offsetY, worldWidth = defaultPreset.params.worldWidth, worldHeight = defaultPreset.params.worldHeight, ...props }) {
  const uniforms = { u_colorBack: getShaderColorFromString(colorBack), u_colorFront: getShaderColorFromString(colorFront), u_shape: DitheringShapes[shape], u_type: DitheringTypes[type], u_pxSize: size, u_fit: ShaderFitOptions[fit], u_scale: scale, u_rotation: rotation, u_offsetX: offsetX, u_offsetY: offsetY, u_originX: originX, u_originY: originY, u_worldWidth: worldWidth, u_worldHeight: worldHeight };
  return jsx2(ShaderMount2, { ...props, speed, frame, fragmentShader: ditheringFragmentShader, uniforms });
});

import { memo as memo2 } from "react";
import { jsx as jsx3 } from "react/jsx-runtime";
var defaultPreset2 = { name: "Default", params: { ...defaultObjectSizing, fit: "cover", speed: 0, frame: 0, colorFront: "#94ffaf", colorBack: "#000c38", colorHighlight: "#eaff94", type: "8x8", size: 2, colorSteps: 2, originalColors: false, inverted: false } };

var ImageDithering = memo2(function ImageDitheringImpl({ speed = defaultPreset2.params.speed, frame = defaultPreset2.params.frame, colorFront = defaultPreset2.params.colorFront, colorBack = defaultPreset2.params.colorBack, colorHighlight = defaultPreset2.params.colorHighlight, image = "", type = defaultPreset2.params.type, colorSteps = defaultPreset2.params.colorSteps, originalColors = defaultPreset2.params.originalColors, inverted = defaultPreset2.params.inverted, pxSize, size = pxSize === void 0 ? defaultPreset2.params.size : pxSize, fit = defaultPreset2.params.fit, scale = defaultPreset2.params.scale, rotation = defaultPreset2.params.rotation, originX = defaultPreset2.params.originX, originY = defaultPreset2.params.originY, offsetX = defaultPreset2.params.offsetX, offsetY = defaultPreset2.params.offsetY, worldWidth = defaultPreset2.params.worldWidth, worldHeight = defaultPreset2.params.worldHeight, ...props }) {
  const uniforms = { u_image: image, u_colorFront: getShaderColorFromString(colorFront), u_colorBack: getShaderColorFromString(colorBack), u_colorHighlight: getShaderColorFromString(colorHighlight), u_type: DitheringTypes[type], u_pxSize: size, u_colorSteps: colorSteps, u_originalColors: originalColors, u_inverted: inverted, u_fit: ShaderFitOptions[fit], u_rotation: rotation, u_scale: scale, u_offsetX: offsetX, u_offsetY: offsetY, u_originX: originX, u_originY: originY, u_worldWidth: worldWidth, u_worldHeight: worldHeight };
  return jsx3(ShaderMount2, { ...props, speed, frame, fragmentShader: imageDitheringFragmentShader, uniforms });
}, colorPropsAreEqual);
...
function useDrift(speed: number) {
  ...
  React2.useEffect(() => {
    ...
    const tick = (now: number) => { if (!start) start = now; const t = (now - start) / 1e3 * speed; setOffset({ x: 0.06 * Math.sin(0.37 * t), y: 0.045 * Math.cos(0.23 * t) }); raf = requestAnimationFrame(tick); };
...
function gradientDataUrl(g: any, aspect: number) {
  ...
  if (!ctx) return "";
  ctx.fillStyle = g.colorDark; ctx.fillRect(0, 0, w, h);
...
function TicketCard({ name, presenter, event, venue, dates, stubText, watermark, width = REF, geometry = TICKET_GEOMETRY, layout = TICKET_LAYOUT, texture = TICKET_TEXTURE, gradient = TICKET_GRADIENT, className }: any) {
  ...
  const scale = fitScale(lines, { availableWidth: perfX - layout.padding * width - 0.03 * width, availableHeight: layout.footerTop * width - layout.nameTop * width - 0.02 * width, fontSize: layout.nameSize * width, lineHeight: layout.nameLead * width, tracking: layout.nameTracking });
  const sourceImage = React2.useMemo(() => texture.engine === "image" ? gradientDataUrl(gradient, geometry.aspect) : "", [texture.engine, gradient, geometry.aspect]);
  const shaderStyle = { position: "absolute", inset: 0, width, height };
  return jsxs("div", { className: `relative select-none ${className ?? ""}`, style: { width, height, clipPath: `path('${ticketClipPath(width, height, geometry)}')` }, children: [
    ...
    jsxs("div", { className: "absolute inset-0", style: { color: layout.inkColor, fontFamily: layout.bodyFont || undefined, ...(layout.engraved ? { textShadow: "0 1px 0 rgba(0,0,0,0.4), 0 -1px 0 rgba(255,255,255,0.15)" } : {}) }, children: [
      ...
      jsx4("div", { className: "absolute whitespace-nowrap uppercase", style: { left: layout.padding * width, top: layout.footerTop * width, fontSize: layout.footerSize * width, letterSpacing: `${layout.footerTracking}em` }, children: venue ? `${venue} \xB7 ${dates}` : dates }),
...
function TiltCard({ children, clipPath, maxTilt = 9, scale = 1.02, glare = 0.16, className }: any) {
  ...
  return jsxs("div", { ref: cardRef, onPointerEnter: () => setHovering(true), onPointerMove: onMove, onPointerLeave: onLeave, className: `relative w-fit will-change-transform ${className ?? ""}`, style: { transition: hovering ? "none" : "transform 420ms cubic-bezier(0.22, 1, 0.36, 1)", transform: "perspective(1200px) rotateX(0deg) rotateY(0deg) scale(1)", transformStyle: "preserve-3d" }, children: [
    children,
...
function AdmitOneTicket({ tilt, ...props }: any) {
  ...
  if (tilt === false) return jsx4(TicketCard, { ...props });
  return jsx4(TiltCard, { clipPath: `path('${ticketClipPath(width, width / geometry.aspect, geometry)}')`, ...tilt, children: jsx4(TicketCard, { ...props }) });
}

function hslToHex(h: number, s: number, l: number) {
  const sat = s / 100; const lig = l / 100;
  const a = sat * Math.min(lig, 1 - lig);
  const channel = (n: number) => { const k = (n + h / 30) % 12; const v = lig - a * Math.max(-1, Math.min(k - 3, 9 - k, 1)); return Math.round(255 * Math.max(0, Math.min(1, v))).toString(16).padStart(2, "0"); };
