    ...
    <>
      <div className="space-y-3 px-6 py-5">
        ...
        <div className="flex gap-2">
          {status === "disabled" && (
            <Button size="sm" variant="outline" onClick={startSetup} disabled={busy}>
              {busy ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : null}
              Enable 2FA
            </Button>
          )}
          {status === "enabled" && (
            <Button size="sm" variant="outline" className="text-destructive hover:bg-destructive/10" onClick={() => setDisableOpen(true)}>
              Disable 2FA
            </Button>
          )}
        </div>
